-- 0024 - trust instructor approval only from a minimum number of answers
--
-- Instructor approval is a percentage of however many learners answered, and the scoring function
-- let that percentage count from a single answer. A quarter of the classes called Good or Excellent
-- rested on fewer than six people answering. Below `thin.min_answers` answers:
--
--   * a passing approval no longer adds its points (a failing one still counts, because when those
--     classes were checked against their recordings most had a real content problem);
--   * the rating must clear `thin.rating_line` on its own, or the band is capped at Average.
--
-- The rule is read from the configuration. No stored configuration carries a `thin` block, so this
-- migration changes no score until a version that does is activated. Same signature as 0015, so no
-- caller changes. Additive and safe to re-run.
