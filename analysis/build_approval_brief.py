...
def before_after_svg():
    ...
    for i, (lab, c) in enumerate((("Today's rule", v1), ("New rule", v2))):
        ...
        p.append('<text x="%d" y="%d" class="lab" text-anchor="end" style="fill:var(--ink)">%s</text>' % (ml - 12, yy + 22, lab))
        wv = vid / mx * pw
        wt = tr / mx * pw
        p.append('<rect x="%d" y="%d" width="%.1f" height="34" rx="4" fill="%s"/>' % (ml, yy, wv, ORANGE))
        p.append('<rect x="%.1f" y="%d" width="%.1f" height="34" rx="4" fill="%s"/>' % (ml + wv + 2, yy, wt, BLUE))
        p.append('<text x="%.1f" y="%d" class="lab" text-anchor="middle" style="fill:#fff">%d</text>' % (ml + wv / 2, yy + 22, vid))
        p.append('<text x="%.1f" y="%d" class="lab" text-anchor="middle" style="fill:#fff">%d</text>' % (ml + wv + 2 + wt / 2, yy + 22, tr))
        p.append('<text x="%.1f" y="%d" class="lab" style="fill:var(--ink2)">%d &middot; $%.0f/mo</text>'
                 % (ml + wv + wt + 10, yy + 22, vid + tr, cost1 if i == 0 else cost2))
    p.append('</svg>')
    return "".join(p)
...
h2{font-size:16.5px;letter-spacing:-.01em;margin:16px 0 6px}
...
.card{background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);padding:10px 14px;margin:8px 0}
.finding{border-left:4px solid var(--warn)}
.ok{border-left:4px solid var(--brand)}
.figure{background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);padding:8px 10px 2px;margin:8px 0}
...
.step{flex:1;background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);padding:8px 11px;font-size:12.2px}
...
.w{flex:1;border-radius:var(--radius);padding:8px 11px;color:#fff}
.w .p{font-size:20px;font-weight:750;letter-spacing:-.02em}
...
<div class="flow">
  ...
  <div class="step"><span class="k">Step 3 &middot; the health score</span><b>Urgent (under %(urgent)d) &middot; Needs a look &middot; Borderline (%(borderline)d+)</b>
    Urgent &rarr; video, always. Borderline &rarr; transcript first. In between &rarr; video if %(reach)d%%+ of the room rated it, else transcript.</div>
</div>

<h2>The Class Health Score &mdash; and why these weights</h2>
<div class="weights">
  <div class="w" style="background:%(ORANGE)s"><div class="p">%(wr).0f%%</div><div class="t"><b>The rating</b> (with today's voice checks). The strongest single signal on every test we ran, so it leads.</div></div>
  <div class="w" style="background:%(BLUE)s"><div class="p">%(wa).0f%%</div><div class="t"><b>The approval vote.</b> Half of it is new information, and it is the only signal that predicts approval trouble &mdash; a real share, not a token one.</div></div>
  <div class="w" style="background:%(GREY)s"><div class="p">%(wt).0f%%</div><div class="t"><b>The instructor's track record</b> (their average over earlier classes). Independent of both; a poor one doubles the risk &mdash; but it describes the instructor, not this class, so it nudges rather than drives.</div></div>
</div>
<p class="sub">Each part scores 100 at its bar and slides to 0 as it gets worse; 100 = every bar met. The data supports
rating 55&ndash;65, vote 25&ndash;35, track record 10&ndash;15 &mdash; %(wr).0f / %(wa).0f / %(wt).0f is the simplest split inside that range.</p>

<div class="card ok">
<p style="margin:0"><b>Why not one score that decides everything?</b> We tried hundreds of weight and threshold
combinations. Every one lets a well-liked instructor's bad rating slide &mdash; the %(line).2f line quietly becomes
about 4.38, the %(abar)d%% bar about 67%%, and <b>%(drop90)d classes</b> we analyse today would be dropped. So the
bars stay hard, and the score does what a weighted average is good at: ordering the queue and choosing the depth.</p>
</div>
...
<h2>The two decisions we need</h2>
<p><b>1.</b> The weights: <b>%(wr).0f / %(wa).0f / %(wt).0f</b>. &nbsp; <b>2.</b> Borderline classes start with a
<b>transcript</b> instead of a video (the one change to today's rule &mdash; a single switch either way).</p>
</div>
...
the %(line).2f line to about 4.38 for liked instructors and dropped %(drop90)d classes we analyse today.</div>
...
<div class="q">Where did 60 / 30 / 10 come from?</div>
<div class="a">Three checks on the same data: how much of each signal is new, which signal predicts the next class,
and what each mix does to the queue. All three put the rating first, the vote at roughly a third, the track record
around a tenth. The weights move only about twenty classes between urgency bands over eight months &mdash; the bars
do the heavy lifting.</div>
