...
import { Kpi } from "@/components/analytics/ui";
import { fetchRatings, byCourse, byMonth, bands, byTopic, bestSmePerTopic, summarize, isBad } from "@/lib/ratings";
import { APPROVAL_BAR, BORDERLINE, GOOD, MIN_VOICES, PARTICIPATION_BAR, URGENT, WEIGHTS } from "@/lib/decision";
...
const VISION: { title: string; body: string; status: "live" | "building" | "planned" }[] = [
  { title: "SME strengths & improvement areas by topic", body: "Per-instructor topic records show where each SME shines and where they need targeted coaching — instead of generic feedback.", status: "live" },
  { title: "Modules that rate low across every instructor", body: "When a module underperforms no matter who teaches it, the problem is the content, structure or difficulty — not the SME. The module watchlist detects exactly this.", status: "live" },
  { title: "Rating & engagement patterns across the course journey", body: "Do ratings start high and decline in later modules? Where does the drop begin, and is it difficulty, fatigue or design? The cohort journeys now read module-sequence data from the sheet's cohort text.", status: "building" },
  { title: "Best SME–module alignment", body: "Historical performance already names the best-known SME per module. Over time this becomes an optimized SME-to-module allocation system.", status: "live" },
  { title: "Learner-level rating analytics", body: "With raw learner-level ratings (via the Metabase integration), we can spot consistently dissatisfied learners, what drives it, and who needs intervention — the learner analytics layer.", status: "planned" },
  { title: "SME performance patterns for hiring, demo evaluation & training", body: "Train the AI on what our best SMEs do — clarity, examples, pacing, engagement — then score hiring demos against those benchmarks and give existing SMEs the same targeted analysis automatically. The analysis engine that reads classes today is the foundation.", status: "planned" },
];
...
export default async function InsightsPage() {
  ...
  const partOf = (list: typeof rows) => avg(list.map((r) => r.participation_pct).filter((v): v is number => v != null).map(Number));
  ...
  const approvalPoints = voted.map((r) => [Number(r.approval_pct), r.rating, isBad(r) || under(r) ? 1 : 0] as [number, number, number]);
  const quadrants = [
  ...
  const scatterPoints = rows.filter((r) => r.participation_pct != null).map((r) => [Number(r.participation_pct), r.rating, isBad(r) ? 1 : 0] as [number, number, number]);
  ...
  const courseCards = courseAgg
    ...
    .map((c) => {
      ...
      const worst = [...monthly].sort((a, b) => (b.badShare ?? 0) - (a.badShare ?? 0) || b.month.localeCompare(a.month))[0];
      const latest = monthly[monthly.length - 1];
      const worstPct = worst ? Math.round((worst.badShare ?? 0) * 100) : 0;
      const latestPct = latest ? Math.round((latest.badShare ?? 0) * 100) : 0;
      const worsening = monthly.length >= 2 && !!worst && !!latest && worstPct > 10 && worst.month === latest.month;
      const sentence =
  ...
  const smeHref = (name: string) => `/team/instructors/${encodeURIComponent(`name:${name}`)}`;
    ...
    <div className="mx-auto max-w-4xl xl:grid xl:max-w-6xl xl:grid-cols-[168px_minmax(0,56rem)] xl:justify-center xl:gap-x-10">
      ...
      <header className="xl:col-start-2">
        ...
        <h1 className="text-[26px] leading-tight font-semibold tracking-[-0.02em] sm:text-[30px]">
          What <span data-numeric>{total.n.toLocaleString()}</span> rated classes actually tell us
        </h1>
        <p className="text-muted-foreground mt-2 max-w-2xl text-[14.5px] leading-relaxed">
          Every live class and test review since January, straight from the ratings sheet — recomputed on every sync, so this page is never out of
          date. <b className="text-foreground">{attended.toLocaleString()}</b> learners sat in a class;{" "}
          <b className="text-foreground">{rated.toLocaleString()}</b> of them rated one
          {total.votes > 0 && total.votes === rated && <> — and every rating came with a vote on whether they would have the instructor back</>}
        ...
        <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-5">
          <Kpi label="Classes rated since January" value={total.n.toLocaleString()} />
          <Kpi label={`Rated below ${GOOD}`} value={<span className="text-destructive">{Math.round((total.badShare ?? 0) * 100)}%</span>} />
          <Kpi label="Would have the instructor back" value={<span className={total.approval != null && total.approval < APPROVAL_BAR ? "text-destructive" : ""}>{fmtPct(total.approval)}</span>} />
          <Kpi label="Typical participation" value={`${Math.round(total.avgParticipation ?? 0)}%`} sub="half the room" />
          <Kpi label="Ever reach 80% participation" value={<span className="text-destructive">{above80Pct.toFixed(1)}%</span>} sub="of classes" />
        </div>
      </header>
      ...
      <article className="min-w-0 xl:col-start-2">
        ...
        <Section id="participation">
          ...
          <p className="text-muted-foreground mb-3 text-[13.5px] leading-relaxed">
            Well-rated and badly-rated classes are rated at almost the same rate — the two groups sit just{" "}
            <b className="text-foreground">{Math.abs(gapPts).toFixed(1)} points apart</b> in average participation. And only{" "}
            <b className="text-foreground">
              {above80} of {total.n.toLocaleString()}
            </b>{" "}
            classes ever reached 80% — which is why the old 80% rule never fired once. The flat cloud below is the whole argument:
          </p>
          <ChartCard
          ...
          <Callout tone="ok" icon={Scale} title="What participation decides" className="mt-3">
            Fewer than <b>{MIN_VOICES} ratings</b> → watch only, whatever the score · below {GOOD} → the class enters the queue · <b>≥ {PARTICIPATION_BAR}%</b> of
            the room rating → a video analysis, under {PARTICIPATION_BAR}% → a transcript read. Every threshold here was measured, not guessed — and section 2
            adds the second signal that decides how urgent each case is.
          </Callout>
        </Section>
        ...
        <Section id="vote">
          ...
          {voted.length === 0 && (
            <p className="text-muted-foreground mb-3 text-[13.5px] leading-relaxed">
              The ratings form also asks whether the room would have the instructor back. No votes have synced yet — this section fills in once the
              sheet&apos;s Yes / No columns arrive.
            </p>
          )}
          {voted.length > 0 && (
            <p className="text-muted-foreground mb-3 text-[13.5px] leading-relaxed">
              Every rating form asks a second question: <i>would you want this instructor to take the class again?</i> Across{" "}
              <b className="text-foreground">{voted.length.toLocaleString()}</b> classes with a vote, <b className="text-foreground">{fmtPct(total.approval)}</b> of{" "}
              {total.votes.toLocaleString()} answers say yes, and in <b className="text-foreground">{Math.round((nobodySaidNo / voted.length) * 100)}%</b> of
              classes nobody said no. The two signals move together but do not agree: the rating flags the class, the vote flags the instructor. Of the{" "}
              <b className="text-foreground">{badVoted}</b> classes rated under {GOOD},{" "}
              <b className="text-foreground">
                {hardGood} ({badVoted ? Math.round((hardGood / badVoted) * 100) : 0}%)
              </b>{" "}
              still cleared the {APPROVAL_BAR}% bar — the room found the class hard but did not blame the teacher. And{" "}
              {noVotesAll > 0 && (
                <>
                  <b className="text-foreground">{Math.round((noVotesInGood / noVotesAll) * 100)}%</b> of every &ldquo;no&rdquo; was cast in a class rated {GOOD} or
                  better —{" "}
                </>
              )}
              <b className="text-foreground">{polite.length}</b> classes were rated fine yet fell under the bar ({politeVoiced} of them with {MIN_VOICES} or more
              votes). Two different questions, two different answers.
            </p>
          )}
          {voted.length > 0 && (
            <>
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                {quadrants.map((q) => (
                  <div key={q.label} className="bg-card shadow-soft h-full rounded-xl border p-4">
              ...
              <Callout tone="ok" icon={Scale} title="Rule v2 — two bars decide if, the Health Score decides how urgent and how deep" className="mt-3">
                Rated below <b>{GOOD}</b>, <i>or</i> fewer than <b>{APPROVAL_BAR}%</b> would have the instructor back (either one, with at least {MIN_VOICES} voices) → the
                class enters the queue. A Class Health Score — rating <b>{Math.round(WEIGHTS.rating * 100)}%</b>, approval vote{" "}
                <b>{Math.round(WEIGHTS.approval * 100)}%</b>, the instructor&apos;s track record <b>{Math.round(WEIGHTS.track * 100)}%</b> — then sets the priority:{" "}
                <b>urgent</b> under {URGENT} goes to video whatever the reach, <b>borderline</b> at {BORDERLINE}+ starts with a transcript, and in between the{" "}
                {PARTICIPATION_BAR}% reach bar decides. A weighted score alone could not replace the bars — it would let a liked instructor&apos;s perfect vote buy
                back a bad rating — so the bars stay hard and the score does the two jobs an average is good at: ordering the{" "}
                <Link href="/team/queue" className="text-primary font-medium hover:underline">
                  queue
                </Link>{" "}
                and choosing the depth. The Class Sentiment Score now carries the same two lines as hard caps, expressed as four bands.
              </Callout>
            </>
          )}
        </Section>
        ...
        <Section id="trend">
          ...
          {worstMonth && first && last && (
            <Callout tone="warn" icon={TrendingDown} title="Low-rated classes are climbing" className="mb-3">
              The share of classes rated below {GOOD} has risen from{" "}
              <b>
                {Math.round((first.badShare ?? 0) * 100)}% in {fmtMonth(first.month)}
              </b>{" "}
              to{" "}
              <b>
                {Math.round((last.badShare ?? 0) * 100)}% in {fmtMonth(last.month)}
              </b>
              ...
              Participation stayed flat all year, so this is a real quality trend — not a change in who fills in ratings.
              {first.votes > 0 && last.votes > 0 && (
                <>
                  {" "}
                  The vote moves on the same slope: classes under the {APPROVAL_BAR}% approval bar went from{" "}
                  <b>
                    {underShare(first)}% in {fmtMonth(first.month)}
                  </b>{" "}
                  to{" "}
                  <b>
                    {underShare(last)}% in {fmtMonth(last.month)}
                  </b>{" "}
                  — the room is not just rating lower; it is more often asking for a different instructor.
                </>
              )}
              {topBadCourse && bad.length > 0 && (
                <>
                  {" "}
                  And it is concentrated: <b>{topBadCourse.name}</b> carries the largest share of the problem —{" "}
                  <b>
                    {topBadCourse.bad} of the {bad.length}
                  </b>{" "}
                  classes rated below {GOOD} this year ({Math.round((topBadCourse.bad / bad.length) * 100)}%).
                </>
              )}
            </Callout>
          )}
          <ChartCard
            ...
            table={{
              ...
              rows: months.map((m) => [fmtMonth(m.month), m.n - m.bad, m.bad, m.badShare != null ? `${Math.round(m.badShare * 100)}%` : "—", fmtPct(m.approval), m.underBar]),
            }}
          >
            <StackedBars
        ...
        <Section id="spread">
          ...
          <p className="text-muted-foreground mb-3 text-[13.5px] leading-relaxed">
            Most classes land at 4.75 or above — <b className="text-foreground">{ratingBands[4]?.count.toLocaleString()}</b> of them. The {GOOD} line separates the
            healthy middle from the <b className="text-foreground">{bad.length}</b> that need a look.
          </p>
          <ChartCard
        ...
        <Section id="courses">
          ...
          <p className="text-muted-foreground mb-3 text-[13.5px] leading-relaxed">
            The trend in section 3 is not spread evenly. Every course with at least <b className="text-foreground">10 rated classes</b>, worst first — the pill is
            its share of classes below {GOOD}, the line is that share month by month, and the vote sits beside the rating.
          </p>
          {courseCards.length === 0 ? (
            <p className="text-muted-foreground text-[13px]">No course has reached 10 rated classes yet — this view fills in as the sheet grows.</p>
          ) : (
            <div className="grid gap-3 sm:grid-cols-2">
              {courseCards.map((c) => (
                <div key={c.key} className="bg-card shadow-soft h-full rounded-xl border p-4">
                  <div className="flex items-start justify-between gap-2">
                    <h3 className="min-w-0 truncate text-[13px] font-semibold" title={c.name}>
                      {c.slug ? (
                        <Link href={hrefIn(c.slug, "/overview")} className="hover:text-primary transition-colors">
                          {c.name}
                        </Link>
                      ) : (
                        c.name
                    ...
                    <span
                      className={cn(
                        ...
                        (c.badShare ?? 0) > (total.badShare ?? 0) ? "bg-destructive/10 text-destructive" : "bg-success/10 text-success",
                      )}
                      data-numeric
                    >
                  ...
                  <p className="text-muted-foreground mt-1 text-xs">
                    <b className="text-foreground font-semibold" data-numeric>
                      {c.n}
                    </b>{" "}
                    classes · avg{" "}
                    <b className="text-foreground font-semibold" data-numeric>
                    ...
                    {c.approval != null && (
                      <>
                        ...
                        would have the instructor back{c.underBar > 0 ? ` · ${c.underBar} under ${APPROVAL_BAR}%` : ""}
                      </>
                    )}
                  </p>
                  {c.spark.length >= 2 && (
                    <div className="mt-2.5 flex items-center gap-2">
                      <Sparkline values={c.spark} width={110} height={26} accent={c.worsening ? "var(--viz-bad)" : "var(--chart-1)"} />
                      <span className="text-muted-foreground text-[10.5px]">% below {GOOD}, by month</span>
                    </div>
                  )}
            ...
            <div className={CARD}>
              <div className="flex items-start gap-2.5 px-4 pt-4 sm:px-5">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-xs">
                    Rated below {GOOD} across <b>two or more different SMEs</b> — pointing at the material, not the person. A healthy approval beside a low rating
                    says the same thing.
                  </p>
                </div>
              </div>
              <Table>
                ...
                <TableBody>
                  {watchlist.map((t) => (
                    <TableRow key={t.topic}>
                      ...
                      <TableNum className={t.approval != null && t.approval < APPROVAL_BAR ? "text-destructive font-semibold" : ""}>
                        {t.approval != null ? (
                          <>
                            <BandDot tone={approvalTone(t.approval)} />
                            {Math.round(t.approval)}%
                          </>
                        ) : (
                  ...
                  {watchlist.length === 0 && (
                    <TableRow>
                      <TableCell className="text-muted-foreground">No multi-SME low modules found — a good sign.</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
            ...
            <div className={CARD}>
              <div className="flex items-start gap-2.5 px-4 pt-4 sm:px-5">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-xs">Highest average among SMEs with repeat classes on the module — the seed of the allocation map.</p>
                </div>
              </div>
              <Table>
                <TableHeader>
                ...
                <TableBody>
                  {bestPairs.map((p) => (
                    <TableRow key={p.topic}>
                      ...
                      <TableCell className="max-w-40 truncate">
                        <Link href={smeHref(p.instructor)} className="hover:text-primary transition-colors">
                          {p.instructor}
                        </Link>
                      </TableCell>
                      <TableNum>
                  ...
                  {bestPairs.length === 0 && (
                    <TableRow>
                      <TableCell className="text-muted-foreground">Needs more repeat SME-module pairs.</TableCell>
                    </TableRow>
                  )}
                </TableBody>
              </Table>
          ...
          <div className="bg-card shadow-soft rounded-xl border p-5 sm:p-6">
            <div className="flex items-start gap-3">
              ...
              <p className="text-muted-foreground text-[13px] leading-relaxed">
                The long-term goal is to move from tracking ratings to a system that decides better — SME allocation, content improvement, learner
                intervention, training, and hiring.
              </p>
            </div>
              ...
              {VISION.map((v, i) => (
                <li key={v.title} className="surface-inset rounded-lg border p-3.5">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <Badge variant={v.status === "live" ? "success" : v.status === "building" ? "warning" : "secondary"} className="shrink-0 capitalize">
                      {v.status}
                    </Badge>
                  </div>
            ...
            <p className="text-muted-foreground mt-4 text-xs leading-relaxed">
              The questions this system will answer: who is the best SME for each topic · where does each SME need improvement · which modules need content
              work · where in the journey do ratings and attendance start declining · which learners are consistently dissatisfied and why · what makes our
              highest-performing SMEs different — and how to use that for hiring, evaluation, training and allocation.
            </p>
          </div>
        </Section>
      </article>
